...
public class PerodiumCowEntity extends PathfinderMob {
	private static final Set<ResourceLocation> SPAWN_BIOMES = Set.of(new ResourceLocation("perodiumcraft:perodium_oak_plains"),
			...
			new ResourceLocation("perodiumcraft:perodium_oak_forest"), new ResourceLocation("perodiumcraft:ruby_jungle_forest"));

	@SubscribeEvent
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.CREATURE)
					.add(new MobSpawnSettings.SpawnerData(PerodiumcraftModEntities.PERODIUM_COW.get(), 20, 4, 4));
	}

	public PerodiumCowEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(PerodiumcraftModEntities.PERODIUM_COW.get(), world);
	}

	public PerodiumCowEntity(EntityType<PerodiumCowEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(PerodiumcraftModItems.PERODIUM_MEAT.get()));
	}
	...
	public static void init() {
		SpawnPlacements.register(PerodiumcraftModEntities.PERODIUM_COW.get(), SpawnPlacements.Type.ON_GROUND,
