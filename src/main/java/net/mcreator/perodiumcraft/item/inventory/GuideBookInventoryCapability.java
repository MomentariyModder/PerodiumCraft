...
package net.mcreator.perodiumcraft.item.inventory;
...
public class GuideBookInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	@SubscribeEvent
	@OnlyIn(Dist.CLIENT)
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == PerodiumcraftModItems.GUIDE_BOOK) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(0) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != PerodiumcraftModItems.GUIDE_BOOK;
			}
