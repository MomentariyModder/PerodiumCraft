...
public class PerodiumLandTeleporter implements ITeleporter {
	...
	public static void registerPointOfInterest(RegistryEvent.Register<PoiType> event) {
		poi = new PoiType("perodium_land_portal",
				Sets.newHashSet(ImmutableSet.copyOf(PerodiumcraftModBlocks.PERODIUM_LAND_PORTAL.getStateDefinition().getPossibleStates())), 0, 1)
				...
				for (int j3 = 0; j3 < 2; ++j3) {
					for (int k3 = -1; k3 < 3; ++k3) {
						BlockState blockstate1 = k3 < 0
								? PerodiumcraftModBlocks.REINFORCED_PERIODIUM_BRICKS.defaultBlockState()
		...
		for (int l1 = -1; l1 < 3; ++l1) {
			for (int j2 = -1; j2 < 4; ++j2) {
				if (l1 == -1 || l1 == 2 || j2 == -1 || j2 == 3) {
					...
					this.level.setBlock(blockpos$mutableblockpos, PerodiumcraftModBlocks.REINFORCED_PERIODIUM_BRICKS.defaultBlockState(), 3);
				}
			}
		}
		BlockState blockstate = PerodiumcraftModBlocks.PERODIUM_LAND_PORTAL.defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
