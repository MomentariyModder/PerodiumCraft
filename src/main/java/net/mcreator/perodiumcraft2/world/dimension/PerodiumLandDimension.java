...
package net.mcreator.perodiumcraft2.world.dimension;
...
import net.mcreator.perodiumcraft2.item.PerodiumLandItem;
import net.mcreator.perodiumcraft2.block.ReinforcedPeriodiumBricksBlock;
import net.mcreator.perodiumcraft2.block.PerodiumStoneBlock;
...
public class PerodiumLandDimension extends Perodiumcraft2ModElements.ModElement {
	@ObjectHolder("perodiumcraft2:perodium_land_portal")
	public static final CustomPortalBlock portal = null;
	public PerodiumLandDimension(Perodiumcraft2ModElements instance) {
		super(instance, 1);
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				ObfuscationReflectionHelper.setPrivateValue(WorldCarver.class, WorldCarver.CAVE, new ImmutableSet.Builder<Block>()
						...
						.add(PerodiumStoneBlock.block.getDefaultState().getBlock()).build(), "field_222718_j");
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("perodiumcraft2:perodium_land"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("perodium_land_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("perodium_land_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())),
					0, 1).setRegistryName("perodium_land_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new PerodiumLandItem().setRegistryName("perodium_land"));
	}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("perodiumcraft2:perodium_land"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("perodiumcraft2:perodium_land")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == ReinforcedPeriodiumBricksBlock.block.getDefaultState().getBlock();
		};
					...
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0
									? ReinforcedPeriodiumBricksBlock.block.getDefaultState().getBlock().getDefaultState()
									: Blocks.AIR.getDefaultState();
				...
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable,
								ReinforcedPeriodiumBricksBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
